...
@Component({
  selector: 'obstacle-caracteristiques',
  templateUrl: './obstacle-caracteristiques.component.html',
  styleUrls: ['./obstacle-caracteristiques.component.scss']
})
export class ObstacleCaracteristiquesComponent implements OnInit {

  @Input() evenement: Evenement;
  @Input() currentCtx: string; // mode d'accès à la ressource (lecture, edition)
  isCollapsedContent: boolean = true;
  field:{ [key: string]: any };
  typesObstacles: string[];
  ...
  ngOnInit() {
    ...
    this.typesObstacles = _.values(ModeleEvenementService.enumerations[EvenementCte.INPUT.TYPES_OBSTACLES].valeursEnumerations);
  }
}
